...
async function loadSettings() {
  ...
  // 同步设置到全局状态
  state.currentMode = extension_settings[extensionName].currentMode || 'online';
  state.streamingEnabled = extension_settings[extensionName].streamingEnabled ?? true;
  state.streamingSpeed = extension_settings[extensionName].streamingSpeed || 1;
  state.isFullScreen = extension_settings[extensionName].isFullScreen || false;

  // 更新UI
  updateModeDisplay();
  dom.streamingToggleItem.textContent = state.streamingEnabled ? '关闭伪流式' : '开启伪流式';
}

// 保存设置变更
function saveExtensionSettings() {
  extension_settings[extensionName].currentMode = state.currentMode;
  extension_settings[extensionName].streamingEnabled = state.streamingEnabled;
  extension_settings[extensionName].streamingSpeed = state.streamingSpeed;
  extension_settings[extensionName].isFullScreen = state.isFullScreen;
...
jQuery(async () => {
  ...
  function updateInputBoxPlaceholder() {
    if (state.character) {
      ...
      dom.inputBox.setAttribute('data-placeholder', `${modeName} - 和${state.character.name}聊天...`);
    } else {
      dom.inputBox.setAttribute('data-placeholder', '请先导入角色卡...');
    }
  ...
  function checkAndShowPlaceholder() {
    ...
    if (!hasText) {
      dom.inputBox.setAttribute('data-empty', 'true');
    } else {
      dom.inputBox.removeAttribute('data-empty');
    }
  }
  ...
  function updateModeDisplay() {
    ...
    dom.modeSwitch.className = `mode-switch mode-${mode}`;
    dom.modeSwitch.innerHTML = config.icon + 
      `<div class=\"mode-indicator\"></div>` +
      `<div class=\"mode-label\">${config.name}</div>`;
    dom.modeSwitch.style.color = config.color;
  ...
  function switchMode() {
    ...
    dom.modeSwitch.classList.add('mode-switch-animate');
    setTimeout(() => {
      dom.modeSwitch.classList.remove('mode-switch-animate');
    }, 400);
  ...
  function handleFullScreenChange() {
    ...
    if (isFullScreen) {
      ...
      dom.fullscreenItem.textContent = '退出全屏';
    } else {
      document.body.classList.remove('fullscreen-mode');
      dom.fullscreenItem.textContent = '全屏模式';
    }
  }
  ...
  function toggleStreaming() {
    ...
    if (state.streamingEnabled) {
      dom.streamingToggleItem.textContent = '关闭伪流式';
    } else {
      dom.streamingToggleItem.textContent = '开启伪流式';
    }
    saveExtensionSettings();
    closeDropdownMenu();
  }
  ...
  function loadAndApplyChatBg() {
    ...
    if (savedBg) {
      ...
      dom.chatList.style.backgroundImage = 'none';
      dom.chatList.style.backgroundColor = 'var(--inner-bg)';
  ...
  function applyChatBg(bgBase64) {
    if (bgBase64 && bgBase64.startsWith('data:image')) {
      const img = new Image();
      img.onload = function() {
        dom.chatList.style.backgroundImage = `url(${bgBase64})`;
        document.querySelectorAll('.bubble').forEach(bubble => {
  ...
  function selectChatBg() {
    dom.chatBgInput.style.zIndex = '9999';
    dom.chatBgInput.click();
  }
  ...
  async function importCharacterToUI(characterData) {
    try {
      ...
      dom.charNameDisplay.innerText = characterData.name;
      dom.emptyTip.style.display = 'none';
      dom.inputBox.removeAttribute('disabled');
      dom.inputBox.focus();
  ...
  function updateStatus(status) {
    ...
    dom.statusDot.style.backgroundColor = config.color;
  ...
  function appendUserMessageStorySeparate(userText, index, replyTo = null) {
    ...
    dom.chatList.appendChild(msgDiv);
    setTimeout(() => {
  ...
  async function regenerateMessage(msgIndex) {
    ...
    if (isUserMsg) {
      ...
      try {
        dom.inputBox.textContent = targetMsg.text;
        dom.footerBar.classList.add('has-text');
  ...
  function addMsgInteractions(element, index) {
    ...
    const clickHandler = (e) => {
      ...
      if (dom.msgActions.style.display === 'block') {
        closeMsgMenu();
      }
  ...
  function showMsgMenu(x, y, index) {
    ...
    dom.msgActions.style.left = adjustedX + 'px';
    dom.msgActions.style.top = adjustedY + 'px';
    dom.msgActions.style.display = 'block';
  ...
  function closeMsgMenu() {
    dom.msgActions.style.display = 'none';
  ...
  function closeMsgMenuOnClick(e) {
    if (!dom.msgActions.contains(e.target)) {
      closeMsgMenu();
    }
  }
  ...
  async function sendMessage() {
    ...
    const text = dom.inputBox.textContent.trim();
    if (!text) return;
    ...
    try {
      const emptyTip = dom.chatList.querySelector('.empty-tip');
      ...
      dom.inputBox.textContent = '';
      dom.footerBar.classList.remove('has-text');
      dom.sendBtn.disabled = true;
...
基于以上信息，请续写故事：`;
  ...
  dom.msgActionDelete.onclick = function() {
    ...
    if (index >= 0) {
      const chatHistory = loadChatHistory();
      if (index < chatHistory.length) {
        const result = deleteChatRecord(index);
  ...
  function setReplyTo(msg, index) {
    ...
    dom.replyPreviewContent.textContent = `回复 ${senderName}: ${previewText}`;
  ...
  function showToast(message, type = 'success') {
    ...
    dom.toast.style.backgroundColor = type === 'error' ? 'rgba(255,59,48,0.8)' : 
  ...
  function init() {
    try {
      ...
      if (userPersona) {
        ...
        console.log('用户设定:', state.userInfo);
      }
      ...
      if (!state.character) {
        dom.inputBox.setAttribute('disabled', 'disabled');
        dom.sendBtn.disabled = true;
      }
    } catch (err) {
  ...
  dom.inputBox.addEventListener('focus', () => {
    checkAndShowPlaceholder();
  });

  dom.inputBox.addEventListener('blur', () => {
    checkAndShowPlaceholder();
  });
